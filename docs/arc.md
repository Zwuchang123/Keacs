...
3. App 根据模型服务模式选择调用官方 Agent 后端或用户自定义模型服务。
4. 官方模式下，App 把用户输入、上下文和设备限流标识发给 Agent 后端。
5. 官方后端校验请求格式，按设备限流，并调用服务端配置的大模型供应商。
6. 自定义模式下，App 直接调用用户配置的访问地址，并使用用户本机保存的 API Key。
7. App 或后端校验模型返回的结构化结果。
8. App 展示回答、追问或待确认操作。
9. 用户确认后，App 通过现有 UseCase 写入本地数据库。
10. 官方模式下，App 把执行成功或失败的元信息反馈给后端；自定义模式不向 Keacs 后端反馈会话内容。
