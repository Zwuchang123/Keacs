...
| 2026-05-12 23:20:00 | 优化默认记账账户上下文、可编辑确认卡片和字段修改反馈事件 | 让助手预览可直接修正，并把真实纠错信号纳入安全自我优化 | Android Agent 模块、Agent 后端、反馈存储、写入安全规则 |
| 2026-05-13 | 修复助手频繁"模型服务不稳定"提示、优化输出长度限制、实现 Markdown 渲染 | 改善用户体验，支持更长回复和格式化的内容展示 | Android Agent UI、后端配置、模型超时和输出配置 |
...
- `agent_feedback_events`：保存点赞、点踩、重新生成、确认卡片字段修改和用户取消等反馈事件。
...
12. App 将点赞、点踩、重新生成、确认卡片字段修改、确认、取消或执行失败结果发回后端。
...
- `AgentFeedbackCollector`：收集点赞、点踩、重新生成、确认卡片字段修改、取消确认和执行失败事件。
...
- 分类列表
- 账户列表
- 默认记账账户标记；未设置默认账户时，不把账户列表第一项视为默认账户
